...
import { RunSummary } from './runs.service';
...
export class WorkflowsService {
  ...
  save(yaml: string, editing = '', saveAs = false): Observable<WorkflowParsed> {
    ...
    const url = editing ? `/api/workflows/${encodeURIComponent(editing)}${query}` : `/api/workflows${query}`;
  ...
  preflight(name: string): Observable<Preflight> {
    return this.http.get<Preflight>(`/api/workflows/${encodeURIComponent(name)}/preflight`);
  }
  ...
  runs(name: string): Observable<RunSummary[]> {
    return this.http.get<RunSummary[]>(`/api/runs?workflow=${encodeURIComponent(name)}`);
  }
   ...
   */
  run(name: string, params: Record<string, string>, files: StagedFile[]): Observable<RunStarted> {
