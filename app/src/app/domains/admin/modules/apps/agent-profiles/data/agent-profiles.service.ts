...
export class AgentProfilesService {
  ...
  get(id: string): Observable<AgentProfileView> {
    return this.http.get<AgentProfileView>(
      `/api/agent-profiles/${encodeURIComponent(id)}`
    );
  }

  create(source: AgentProfileSave): Observable<{ id: string }> {
    return this.http.post<{ id: string }>('/api/agent-profiles', source);
  }

  update(id: string, source: AgentProfileSave): Observable<{ id: string }> {
