...
export class RunsService {
  ...
  page(filter: RunFilter = {}): Observable<RunPage> {
    ...
    return this.http
      .get<RunSummary[]>('/api/runs', { params, observe: 'response' })
      .pipe(
        map((response) => ({
          runs: response.body ?? [],
  ...
  live(): Observable<RunSummary[]> {
    return this.page({ state: ['ACTIVE', 'WAITING_ON_CHILD', 'ASKED'] }).pipe(
      map((p) => p.runs)
    );
  }
  ...
  result(taskId: string): Observable<TaskResultView> {
    return this.http.get<TaskResultView>(
  ...
  abandon(runId: string): Observable<RunSummary> {
    return this.http.post<RunSummary>(
  ...
  answer(
    ...
    const body: { message: string; secrets?: Record<string, string> } = {
      message,
    };
