 ...
 * class and a lucide icon. Keep this the single source of truth for state styling.
 */
...
export function statusView(
  ...
  if (
    runState === 'ABANDONED' &&
    (state === 'REAPED' || state === 'ABANDONED')
  ) {
    return {
      label: 'Stopped',
      badge: 'text-secondary bg-white/10',
      icon: 'circle-slash',
    };
  }
  switch (state) {
    ...
    case 'PENDING':
      return {
        label: 'Pending',
        badge: 'bg-white/10 text-secondary',
        icon: 'clock',
      };
    case 'FAILED':
    case 'REAPED':
