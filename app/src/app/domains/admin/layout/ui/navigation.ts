...
import { Attention } from '@/app/core/attention';
import {
  NAVIGATION,
  NavigationItem,
...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-y-4">
      @for (section of decorated(); track section.id) {
...
export class Navigation {
  ...
  private attention = inject(Attention);

  // State
  /** Expansion state — written back when the active route is expanded on load. */
  private navigation = signal<NavigationItem[]>(NAVIGATION);

  /**
   * The nav as rendered: expansion state with the Live badge layered on.
   *
   * Derived rather than written into the data, so the count can change every ten seconds without
   * anything having to remember to keep the static navigation in step.
   */
  protected decorated = computed<NavigationItem[]>(() => {
    const waiting = this.attention.waiting();
