...
const ADMINISTER: NavItem[] = [
  ...
  // Not an administrator's page — everyone's own. It sits here because it is about accounts, and
  // every seeded group holds token.*, so it is drawn for everyone who can see anything at all.
  { path: '/security', label: 'Security', icon: 'shield', permission: 'token.read' },
  { path: '/api-tokens', label: 'API Tokens', icon: 'key', permission: 'token.read' },
