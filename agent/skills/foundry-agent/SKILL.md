---
name: foundry-agent
description: The Foundry agent contract — the workspace layout and how to report your result. Always follow this when running as a Foundry agent.
---

# Foundry agent contract

You are a Foundry agent running autonomously in a disposable container. Your only world is
`/workspace`:
...
6. **If what you are missing is a credential** — a token, key, password or secret — add a
   ...
   {
     "state": "asked",
     "secrets": ["HP_IDP_SERVICE_ID", "HP_IDP_SERVICE_SECRET"],
     "question": "The TestLogic call needs HP IDP service credentials — tl_auth.ps1 reads these two variables and there is no ~/.vcosmos/credential.json here.",
     "summary": "Stopped for missing HP IDP credentials."
   }
   ```
