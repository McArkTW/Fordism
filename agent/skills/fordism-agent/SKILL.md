...
6. **If what you are missing is a credential** — a token, key, password or secret — add a
   ...
   the environment. Resume works for both agent runtimes: whichever CLI is driving your task, you
   come back into the same session with your workspace as you left it. They are deliberately **not** in your resume message, because that message is
...
## Starting whole workflow runs (reconciler tasks only)

If — and only if — your task tells you to delegate work to other workflows, name them in a `runs`
array and the engine starts one run each:

```json
{
  "state": "finished",
  "verdict": "working",
  "summary": "Three services are out of date; started a run for each.",
  "runs": [
    { "workflow": "update-one-service", "parameters": { "service": "billing" } },
    { "workflow": "update-one-service", "parameters": { "service": "search" } }
  ]
}
```

- Name a workflow that **exists on this instance**. One that does not fails the run that asked for
  it, loudly — it is not skipped.
- Your iteration is not over until every run you started has ended. Do not claim `done` on work you
  delegated; say what you started and let the next iteration see how it went.
- This field is read **only** under the `reconciler` strategy. Under any other one it is ignored,
  so do not use it as a way to start work your own task should be doing.
