  ...
  template: `
    <div class="mx-auto w-full max-w-6xl p-6">
      <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <a class="btn btn-primary" routerLink="/workflows/new"><app-icon name="plus" />New workflow</a>
      </div>
          ...
          @for (w of shown(); track w.name) {
            <section class="card p-4 transition hover:border-muted">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <a class="btn px-2 py-1 text-xs" [routerLink]="['/workflows', w.name, 'run']"><app-icon name="play" />Run</a>
                <a class="btn px-2 py-1 text-xs" [routerLink]="['/workflows', w.name, 'edit']"><app-icon name="pencil" />Edit</a>
              </div>
              @if (w.description) {
        ...
        @if (shown().length === 0) {
          <div class="card mt-4 border-dashed px-6 py-16 text-center">
            @if (all().length === 0) {
              ...
              <a class="btn btn-primary mt-4" routerLink="/workflows/new"><app-icon name="plus" />Create your first workflow</a>
            } @else {
