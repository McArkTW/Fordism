#!/usr/bin/env bash
# Deploy a Foundry environment. Run by the self-hosted CI runner on the target,
...
for i in $(seq 1 30); do
  if docker compose --project-name "$PROJECT" --env-file "$ENV_FILE" -f docker-compose.yml \
       exec -T foundry-core curl -fsS http://localhost:8080/api/health >/dev/null 2>&1; then
