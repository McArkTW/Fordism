...
public final class RoutePermissions {
    ...
    private static final List<Rule> RULES = List.of(
            ...
            new Rule(HandlerType.DELETE, "/api/workflows/**", Permission.WORKFLOW_DELETE),
            ...
            new Rule(HandlerType.GET, "/api/skills-file", Permission.SKILL_READ),
            new Rule(HandlerType.GET, "/api/skills/**", Permission.SKILL_READ),
            ...
            new Rule(HandlerType.POST, "/api/skills-delete", Permission.SKILL_WRITE),
            new Rule(HandlerType.POST, "/api/skills/**", Permission.SKILL_WRITE),
            new Rule(HandlerType.DELETE, "/api/skills/**", Permission.SKILL_WRITE),
            // A plugin points this instance at a URL and stages whatever comes back into the
            // library every agent reads — a different power from editing a SKILL.md, so it has its
            // own permission. Named under skill, so a group holding skill.* is unaffected.
            new Rule(HandlerType.GET, "/api/skill-plugins", Permission.SKILL_READ),
            new Rule(HandlerType.POST, "/api/skill-plugins", Permission.SKILL_PLUGIN_WRITE),
            new Rule(HandlerType.POST, "/api/skill-plugins/**", Permission.SKILL_PLUGIN_WRITE),
            new Rule(HandlerType.DELETE, "/api/skill-plugins/**", Permission.SKILL_PLUGIN_WRITE),
            ...
            new Rule(HandlerType.POST, "/api/runs/*/abandon", Permission.RUN_ABANDON),
            ...
            // ---- your own API tokens (the gate also refuses a token-borne call here) ----
            new Rule(HandlerType.GET, "/api/api-tokens", Permission.TOKEN_READ),
            new Rule(HandlerType.POST, "/api/api-tokens", Permission.TOKEN_WRITE),
            new Rule(HandlerType.DELETE, "/api/api-tokens/**", Permission.TOKEN_WRITE),

            // ---- the audit trail (admins only — it names every actor) ----
            new Rule(HandlerType.GET, "/api/audit", Permission.AUDIT_READ));
