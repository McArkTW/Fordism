...
public record User(String id, String email, String displayName, String passwordHash,
                   List<LinkedIdentity> identities, Mfa mfa) {
    ...
    public User {
        ...
        // A null mfa is "not enrolled" — the shape an account written before MFA existed restores as.
    }
    ...
    public static User withPassword(String email, String displayName, PasswordHash password) {
        return new User(null, email, displayName, password.encoded(), List.of(), null);
    }
    ...
    public static User fromProvider(ExternalIdentity identity) {
        return new User(null, identity.email(), identity.displayName(), "", List.of(identity.link()), null);
    }

    /** Whether this account has a second factor enrolled. Its secret never leaves the server. */
    public boolean mfaEnabled() {
        return mfa != null && mfa.enabled();
    }

    /** The same account with a second factor enrolled (or replaced). */
    public User withMfa(Mfa enrolled) {
        return new User(id, email, displayName, passwordHash, identities, enrolled);
    }

    /** The same account with its second factor removed. */
    public User withoutMfa() {
        return new User(id, email, displayName, passwordHash, identities, null);
    }
    ...
    public User withId(String assigned) {
        return new User(assigned, email, displayName, passwordHash, identities, mfa);
    }

    public User withPasswordHash(PasswordHash password) {
        return new User(id, email, displayName, password.encoded(), identities, mfa);
    }
    ...
    public User linkedTo(LinkedIdentity identity) {
        ...
        linked.add(identity);
        return new User(id, email, displayName, passwordHash, linked, mfa);
    }
