...
public final class SkillPluginController {
    ...
    /**
     * DELETE /api/skill-plugins/{id}[?keepSkills=true] — forget it, and delete the skills it
     * installed unless asked to keep them.
     *
     * <p>Deleting is the default because that is what the button did before this flag existed: a
     * caller that has not been updated must not start keeping folders it expected to be cleaned up.
     */
    public void remove(Context ctx) {
        try {
            plugins.remove(ctx.pathParam("id"), "true".equalsIgnoreCase(ctx.queryParam("keepSkills")));
            ctx.status(204);
        } catch (IllegalArgumentException e) {
            Api.fail(ctx, 404, e.getMessage());
        } catch (Exception e) {
