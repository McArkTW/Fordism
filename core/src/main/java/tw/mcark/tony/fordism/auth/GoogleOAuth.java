...
public final class GoogleOAuth implements ExternalAuthProvider {
    ...
    public GoogleOAuth(AuthConfiguration configuration, HttpClient http, IdTokenVerifier idTokens) {
    ...
    public Optional<ExternalIdentity> identify(LoginCallback callback) {
        ...
        return idTokens.verify(response.get("id_token").getAsString(), callback.attempt().nonce())
                .flatMap(GoogleOAuth::identityFrom);
    }

    /**
     * Google's rule for a trustworthy address, applied here rather than in the verifier because it
     * is Google's and not every issuer's.
     *
     * <p>{@code email_verified} is the whole of it: without that check an attacker could claim a
     * colleague's address by putting it on an unverified Google account and letting the allowlist
     * do the rest.
     */
    static Optional<ExternalIdentity> identityFrom(VerifiedClaims claims) {
        if (claims.email().isBlank() || !claims.emailVerified()) {
            Logger.warn("rejected a Google id_token: it carries no verified email");
            return Optional.empty();
        }
        String name = claims.displayName().isBlank() ? claims.email() : claims.displayName();
        return Optional.of(new ExternalIdentity(AuthProviderId.GOOGLE, claims.subject(),
                claims.email(), name));
    }
}
