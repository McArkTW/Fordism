...
public final class UserController {
    ...
    public void create(Context ctx) {
        try {
            ...
            User draft = new User(null, Api.string(body.get("email")), Api.string(body.get("displayName")),
                    password.isEmpty() ? "" : PasswordHash.of(password).encoded(), List.of(), null);
    ...
    public void update(Context ctx) {
        ...
        try {
            ...
            User edited = new User(existing.id(),
                    ...
                    existing.passwordHash(), existing.identities(), existing.mfa());
    ...
    public void delete(Context ctx) {
        ...
        // A token outlives a browser by design, so it has to be revoked explicitly — an account
        // deleted while one of its tokens was in a CI job would otherwise keep answering to it.
        accounts.apiTokens().revokeUser(id);
